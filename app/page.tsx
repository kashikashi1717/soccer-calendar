...
export default function SoccerCalendarApp() {
  ...
  const [type, setType] = useState("0"); // 数字で管理
  ...
  const getTypeConfig = (t: any) => {
    // 文字列でも数字でも判定できるようにする
    if (t === "0" || t === "練習") return { label: "練", full: "練習", color: "bg-gray-500" };
    if (t === "1" || t === "トレマ") return { label: "ト", full: "トレマ", color: "bg-green-600" };
    if (t === "2" || t === "リーグ戦") return { label: "リ", full: "リーグ戦", color: "bg-blue-600" };
    if (t === "3" || t === "他試合") return { label: "他", full: "他試合", color: "bg-orange-500" };
  ...
  const addGame = async () => {
    ...
    setDate(""); setOpponent(""); setLocation(""); setType("0"); setMemo("");
  };

  const deleteGame = async (id: string) => {
    if (confirm("削除しますか？")) await deleteDoc(doc(db, "games", id));
  };
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    cells.push(
      <Card key={d} onClick={() => setSelectedDate(dateStr)} className={`p-1 min-h-[85px] cursor-pointer ${dayGames.length > 0 ? 'bg-blue-50/20 border-blue-200' : ''}`}>
        <div className="text-[10px] font-black border-b border-gray-100 mb-1">{d}</div>
  ...
  return (
    <div className="p-2 md:p-4 max-w-4xl mx-auto bg-white min-h-screen text-slate-900">
      <h1 className="text-xl font-black mb-4 text-center">⚽ 試合カレンダー（蒼一朗）</h1>
      ...
      <div className="grid grid-cols-7 gap-1 mb-6">
        {["日", "月", "火", "水", "木", "金", "土"].map((d, i) => (
          <div key={d} className={`text-center text-[10px] font-black ${i === 0 ? "text-red-500" : i === 6 ? "text-blue-500" : "text-slate-400"}`}>{d}</div>
        ))}
        {cells}
      </div>
      ...
      {selectedDate && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setSelectedDate(null)}>
          <div className="bg-white w-full max-w-sm rounded-3xl p-6 space-y-4" onClick={e => e.stopPropagation()}>
            <h3 className="text-xl font-black border-b pb-2">{selectedDate.replace(/-/g, "/")}</h3>
            <div className="max-h-[50vh] overflow-y-auto space-y-4">
              {gamesByDate[selectedDate]?.map((g: any) => (
                <div key={g.id} className="p-4 rounded-2xl border bg-slate-50 relative">
                  <div className={`inline-block px-3 py-1 rounded-full text-[10px] font-black mb-2 text-white ${getTypeConfig(g.type).color}`}>{getTypeConfig(g.type).full}</div>
                  <div className="text-lg font-black">vs {g.opponent}</div>
                  <div className="text-sm text-slate-600">📍 {g.location}</div>
                  <div className="text-sm text-slate-500">⏰ {g.date.split("T")[1]}</div>
                  {g.memo && <div className="mt-2 p-2 bg-white rounded border text-xs text-slate-700 whitespace-pre-wrap">{g.memo}</div>}
            ...
            <Button onClick={() => setSelectedDate(null)} className="w-full py-4 bg-slate-900">閉じる</Button>
          </div>
        </div>
      )}

      <Card className="p-5 max-w-md mx-auto bg-slate-50 border-none ring-1 ring-slate-200">
        <h2 className="text-sm font-black mb-4 text-slate-400 uppercase">予定を追加</h2>
